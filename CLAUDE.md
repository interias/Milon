...
**Offen Phase 2:** echter Drive-Pull der HC-Zip (rclone/OAuth) statt manuellem Ablegen.
...
- **Mobil-Zugriff via Next-Proxy:** Frontend nutzt relativen `/api`-Pfad (`api.ts` BASE), `next.config.ts` `rewrites` proxien `/api/:path*` → `http://127.0.0.1:8000`. Damit erreicht das Handy (`http://<PC-IP>:3000`) das Backend **ohne CORS und ohne Firewall-Freigabe für :8000** (nur Node/:3000 muss erreichbar sein). Hintergrund: Netzwerkprofil **Public**, venv-`python.exe` hat keine Inbound-Regel. CORS-Regex (LAN-IPs) bleibt als Fallback. `mediaUrl` ist jetzt `/api/media/progress/...` (proxied, verifiziert). Backend-Task bindet `0.0.0.0`.
  - **Coach-Lade-Animation (2026-06-24):** Während der Coach rechnet (`/coach/ask|daily|weekly`, 20–40 s) zeigt `components/CoachThinking.tsx` ein zentriertes Overlay-Modal mit einem **animierten Inline-SVG-Läufer** (Beine/Arme scheren via CSS-Keyframes, Boden scrollt; `prefers-reduced-motion`-sicher) + **rotierenden Gags** aus `lib/quips.ts` (100 Sprüche, alle 2,8 s zufällig, Fade). Esc/Klick blendet aus (Request läuft weiter), „läuft seit Xs". Eingehängt in `app/coach/page.tsx` via `busy`-State (`<CoachThinking busy={busy} />`). E2E verifiziert.
